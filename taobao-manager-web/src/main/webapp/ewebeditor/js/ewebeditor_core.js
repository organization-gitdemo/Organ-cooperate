                ...
                case "fontname":
                    ...
                    for (var i = 0; i < lang["FontNameItem"].length; i++) {
                        if (lang["FontNameItem"][i] == v) {
                            G += dY("", "fq('face','" + lang["FontNameItem"][i] + "')", 120, lang["FontNameItem"][i]);
                        } else {
                            G += dY("", "fq('face','" + lang["FontNameItem"][i] + "')", 119, lang["FontNameItem"][i]);
                        }
                    }
                    break;
                case "formatblock":
...
function exec(iA, iQ) {
    ...
    Init: function () {
        ...
        this.hW = document.location.protocol.toLowerCase() + "//" + document.location.host;
...
function DN() {
    ...
    if (config.FixWidth) {
        if (F.Ei) {
            ...
            cR.style.minHeight = (ifr_Editarea.clientHeight - DI - DE - DA - EL - En - EC) + "px";
        }
    } else {
        if (F.Ei && F.as) {
...
function setHTML(V, ub) {
    ...
    switch (EWEB.ae) {
        ...
        case "EDIT":
            ...
            if (F.as) {
                if (config.FixWidth) {
                    R.az(EWEB.T.getElementById("eWebEditor_FixWidth_DIV"), 'beforedeactivate', function () {
                        C.Save();
                    });
                } else {
...
function kZ(el) {
    var sl = ["p", "div", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "ol", "ul", "pre", "fieldset", "form", "table", "blockquote", "dl", "li", "br"];
...
function pb(hv, V) {
    ...
    if (V.length > ga) {
        ...
        while (V.length > 0) {
            var fV;
            if (F.as) {
                fV = mU.createElement('<textarea name="' + ny + '"></textarea>');
            } else {
...
function wr() {
    ...
    bC += "<form id='eWebEditor_UploadForm' action='" + config.ServerExt + "/upload." + config.ServerExt + "?action=remote&type=remote&style=" + aA.StyleName + "&language=zh-cn&cusdir=" + aA.CusDir + "&skey=" + aA.SKey + "' method='post' target='eWebEditor_UploadTarget'>";
...
function qy(qH, sB) {
    var Cw = false;
    var aI = "<table border='0' cellpadding='0' cellspacing='0' width='100%' id='eWebEditor_Toolbar_" + sB + "' unselectable>";
    for (var i = 0; i < qH.length; i++) {
        ...
        for (var j = 0; j < tb.length; j++) {
            ...
            if (aM == "TBSep") {
                ...
                switch (aM) {
                    case "FontName":
                        ...
                        for (var k = 0; k < lang[aM + "Item"].length; k++) {
                            bW += "<option value='" + lang[aM + "Item"][k] + "'>" + lang[aM + "Item"][k] + "</option>";
                        }
                        break;
                    case "FontSize":
                        fS = " style='width:55px'";
                ...
                aI += "<td class='TB_Btn_Padding'><select name='TB_Name_" + aM + "' onchange=\"" + hL + "\" size=1 " + fS + "><option selected>" + lang[aM] + "</option>" + bW + "</select></td>";
            }
        }
...
aE.vb = function (V) {
    function _Replace(m, aH) {
        function gm(m, bv) {
            if (['flash', 'flv', 'mediaplayer6', 'mediaplayer7', 'realplayer', 'quicktime', 'unknownobject'].IndexOf(aH) >= 0) {
                var ak = aE.pr(m, 'img');
                if (ak != '') {
                    ak = ' style=' + ak;
                }
...
aE.pa = function (V, aH, bL) {
    function _Replace(m, m1, m2, m3, m4, m5) {
        if (/_ewebeditor_ta_/.test(m) || /_ewebeditor_fake_/.test(m)) {
            ...
            return m1 + m2 + ' _ewebeditor_ta_' + bL + '="' + encodeURIComponent(m4.replace(/\"/gi, "'")) + '"' + m5;
        }
...
aE.sO = function (V) {
    function _Replace(m, bL, bv) {
        ...
        var s = m.replace(r, '');
        r = new RegExp('\\s_ewebeditor_ta_' + bL + '+\\s*?=\\s*?\"[^\"]*?\"', 'gi');
        return s.replace(r, ' ' + bL + '="' + decodeURIComponent(bv) + '"');
    };
    return V.replace(/<\w+[^>]*?_ewebeditor_ta_(\w+)\s*?=\s*?\"([^\">]*?)\"[^>]*>/gi, _Replace);
};
aE.sG = function (V) {
    ...
    if (F.as) {
        ...
        for (var i = 0; i < hF.length; i++) {
            ...
            if (fH.inRange(eo)) {
                ...
                if (((fH.compareEndPoints("StartToEnd", eo) < 0) && (fH.compareEndPoints("StartToStart", eo) > 0)) || ((fH.compareEndPoints("EndToStart", eo) > 0) && (fH.compareEndPoints("EndToEnd", eo) < 0))) {
                    return true;
                }
            }
        }
...
function dB(aN) {
    ...
    switch (aN) {
        ...
        case "br":
            ...
            break;
        case "code":
            insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#FDFDDF style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">' + lang["HtmlCode"] + '</font><br>' + fn(bE) + '</td></tr></table>');
            break;
        case "quote":
            insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#F3F3F3 style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">' + lang["HtmlQuote"] + '</font><br>' + fn(bE) + '</td></tr></table>');
            break;
        case "big":
    ...
    Options: [10, 25, 50, 75, 100, 150, 200, 500], qr: 100, Execute: function (kX) {
        if (F.as) {
...
C.ai = function () {
    if (F.as) {
        ...
        var type = 'Text';
        var At = {img: 1, hr: 1, li: 1, table: 1, tr: 1, td: 1, embed: 1, object: 1, ol: 1, ul: 1};
        var bk;
        try {
            bk = this.bG();
        } catch (e) {
        ...
        if (bk && bk.rangeCount == 1) {
            ...
            if (range.startContainer == range.endContainer && (range.endOffset - range.startOffset) == 1 && range.startContainer.nodeType == 1 && At[range.startContainer.childNodes[range.startOffset].nodeName.toLowerCase()]) {
                type = 'Control';
            }
        }
        return type;
